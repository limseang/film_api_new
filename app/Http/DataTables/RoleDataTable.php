...
class RoleDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($table) {
                return view('role.action', ['table' => $table]);
            })
            ...
            ->editColumn('status', function ($table) {
                $publish_status = ($table->status == 'Y') ? '<span class="'.config('setup.badge_success').'">'.trans('sma.publish_yes').'</span>' : '<span class="'.config('setup.badge_danger').'">'.trans('sma.publish_no').'</span>';
    ...
    public function query(Role $model): QueryBuilder
    {
        ...
        $model->select(['id','name', 'description', 'created_at' ]);
        if (request('name')) {
