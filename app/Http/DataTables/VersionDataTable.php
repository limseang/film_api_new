...
class VersionDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($table) {
                return view('version.action', ['table' => $table]);
            })
            ->editColumn('created_at', function ($table) {
                return dateTimeFormat($table->created_at);
            })
            ->editColumn('platform', function ($table) {
                return '<span class="'.config('setup.badge_primary').'">'. $table->platform .'</span>';
            })
            ...
            ->rawColumns(['status', 'icon','platform']) #allowed for using html code here
        ;
    }
     ...
     */
    public function query(VersionCheck $model): QueryBuilder
    {
        $model = $model->newQuery();
        $model->select(['id','version', 'platform', 'status', 'created_at' ]);
        if (request('name')) {
            $model->where(function ($query) {
                $query->orWhere('platform', 'like', '%' . request('name') . '%')
                    ->orWhere('version', 'like', '%' . request('name') . '%');
            });
        }
        if (request('publish')) {
