...
//            dd($fcmToken);
   ...
   // Todo: Admin
    ...
    public function verifyTelegramData($data)
    {
        ...
        // Step 1: Prepare the check string
        // Ensure we exclude 'hash' from the check string and sort keys correctly
        $checkString = collect($data)
            ->except('hash') // Exclude the 'hash' field from the calculation
        ...
        // Step 2: Create the secret key using your bot's token
        // The secret key is generated by hashing the bot token with SHA256
        $secretKey = hash('sha256', $botToken, true);

        // Step 3: Generate the hash for comparison
        $generatedHash = hash_hmac('sha256', $checkString, $secretKey);

        // Log the generated and received hashes
