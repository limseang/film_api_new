    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <form action="{{ route('artical.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        @if(session('error'))
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="5" required>{{ old('description', $article->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('origin_id') is-invalid @enderror" id="origin_id" name="origin_id" required>
                                ...
                                @foreach($origins as $origin)
                                    <option value="{{ $origin->id }}" {{ old('origin_id', $article->origin_id) == $origin->id ? 'selected' : '' }}>
                                        {{ $origin->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            @if($article->image)
                                <div class="mb-2">
                                    <img src="{{ $article->image }}" alt="{{ $article->title }}" style="max-width: 200px;">
                                </div>
                            @endif
