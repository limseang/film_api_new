        ...
        <div class="card-body">
          <div class="row">
            <div class="col-12 col-lg-8 p-10">
              ...
              <form method="POST" action="{{route('tag.update', $tag->id)}}" class="needs-validation" novalidate>
                @csrf
                <div class="mb-3">
                  <label class="form-label" for="name">{{__('sma.name')}}</label>
                  <input type="text" class="form-control" name="name" value="{{$tag->name}}" id="name" placeholder="Enter name" required>
                ...
                <select id="publish" class="{{ config('setup.input_select2') }}" name="status">
                    ...
                    <option value="1" {{$tag->status == '1' ? 'selected':''}}>{{ __('global.publish_yes') }}</option>
                    <option value="2" {{$tag->status == '2' ? 'selected':''}}>{{ __('global.publish_no') }}</option>
                </select>
                </div>
                <div class="mb-3">
                  <label class="form-label" for="type">{{__('sma.type')}}</label>
                  <input type="text" class="form-control" name="type" value="{{$tag->type}}" id="type" placeholder="Enter type" required>
                  <span class="invalid-feedback">
                    The field is required.
                  </span>
                </div>
