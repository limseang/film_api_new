        ...
        <div class="card-body">
          <div class="row">
            <div class="col-12 col-lg-8 p-10">
              ...
              <form method="POST" action="{{route('type.update', $type->id)}}" class="needs-validation" novalidate>
                @csrf
                <div class="mb-3">
                  <label class="form-label" for="name">{{__('sma.name')}}</label>
                  <input type="text" class="form-control" name="name" value="{{$type->name}}" id="name" placeholder="Enter name" required>
                ...
                <select id="publish" class="{{ config('setup.input_select2') }}" name="status">
                    ...
                    <option value="1" {{$type->status == '1' ? 'selected':''}}>{{ __('global.publish_yes') }}</option>
                    <option value="2" {{$type->status == '2' ? 'selected':''}}>{{ __('global.publish_no') }}</option>
                </select>
                </div>
                <div class="mb-3">
